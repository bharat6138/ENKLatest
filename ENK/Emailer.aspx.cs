    ...
    public partial class Emailer : System.Web.UI.Page
    {
        ...
        protected void btnSend_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                this.SendHtmlFormattedEmail(SendTo, Subject, body);
            }
            catch (Exception ex)
            {
        ...
        private void SendHtmlFormattedEmail(string recepientEmail, string subject, string body)
        {
            using (MailMessage mailMessage = new MailMessage())
            {
                ...
                string MailAddress = ConfigurationManager.AppSettings.Get("Fromail");
                string PassWord = ConfigurationManager.AppSettings.Get("Password");
                string TOBCC = ConfigurationManager.AppSettings.Get("TOBCC");
        ...
        private string PopulateBody()
        {
            ...
            {
                using (StreamReader reader = new StreamReader(Server.MapPath("~/ENKEmailer.htm")))
                {
            ...
            // BNK
            else if(ddlClient.SelectedValue == "2")
            {
                using (StreamReader reader = new StreamReader(Server.MapPath("~/BNKEmailer.htm")))
                {
                    body = reader.ReadToEnd();
                }
            }
